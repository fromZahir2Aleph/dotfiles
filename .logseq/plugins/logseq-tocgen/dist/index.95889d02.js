...
async function gotoBlock(pageName, blockUUID, count = 0) {
...
function Block({ block , page , blocksToHighlight , path , setData , refetchData ,  }) {
    const goTo = T$1((e)=>{
        if (e.shiftKey) {
            ...
            gotoBlock(page.name, block.name != null && block.children[0] ? block.children[0].uuid : block.uuid);
        }
    }, [
        page,
        block
...
function TocGen({ slot: slot1 , uuid , root: root1 , height , levels , headingType: headingType1 , blocksToHighlight , pushRoot , removeRoots ,  }) {
    ...
    const goToPage = T$1((e)=>{
        if (e.shiftKey) {
            ...
            gotoBlock(page1.name, root1.uuid);
        }
    }, [
        page1,
        root1
...
async function main() {
  ...
  `);
    ...
    routeOff = logseq.App.onRouteChanged(async ({ template , parameters: { query  }  })=>{
        if (lastPageUUID) {
            if (lastScrollTop > 0) {
                await logseq.Editor.upsertBlockProperty(lastPageUUID, "scroll-top", lastScrollTop);
            } else {
                await logseq.Editor.removeBlockProperty(lastPageUUID, "scroll-top");
            }
        }
        lastPageUUID = null;
        if (template !== "/page/:name" || query.anchor && query.anchor.startsWith("block-content-") && query.anchor !== "block-content-editor") return;
    ...
    logseq.useSettingsSchema([
        ...
        },
        {
            ...
            default: "top",
            description: t$2("Defines where to scroll to when you open a page. 'top' goes to the top of the page, 'last' goes to the position where you last leave the page.")
        },
        {
            key: "maxScrollTryCount",
            type: "number",
            default: 20,
            description: t$2("Specify a maximum number of times to scroll down to load lazy content.")
        }, 
    ]);
